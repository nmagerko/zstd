...
static int usage_advanced(const char* programName)
{
    ...
    DISPLAY( "--size-hint=# : optimize compression parameters for streaming input of the given size \n");
...
int main(int argCount, const char* argv[])
{
        ...
        if (nextArgumentsAreFiles==0) {
            ...
            if (argument[0]=='-') {
                ...
                if (argument[1]=='-') {
                    ...
                    if (longCommandWArg(&argument, "--size-hint=")) { hintedSrcSize = readU32FromChar(&argument); continue; }
    ...
    if (operation==zom_compress) {
        ...
        FIO_setHintedSrcSize(prefs, hintedSrcSize);
...
#else
        (void)suffix; (void)adapt; (void)rsyncable; (void)ultra; (void)cLevel; (void)ldmFlag; (void)literalCompressionMode; (void)hintedSrcSize; (void)targetCBlockSize; /* not used when ZSTD_NOCOMPRESS set */
